...
ANOVA supone datos normales y varianzas parecidas, y si eso no se cumple su p-value no es del todo confiable, Kruskal-Wallis no supone nada de eso, entonces, para saber a cuál de las dos rutas creerle, había que mirar primero los datos: Shapiro-Wilk para normalidad y Levene para varianzas. En el material (data_analysis.org) esas revisiones están comentadas dentro de su función anova(), así que en cierta forma solo se completó lo que ya estaba ahí.

Un pequeño detalle: Shapiro no acepta más de 5000 datos de golpe y el grupo de EEUU tiene 5071, así que se sacó una muestra de 500 por grupo con semilla fija (42) para que el resultado salga igual en cada ejecución.
